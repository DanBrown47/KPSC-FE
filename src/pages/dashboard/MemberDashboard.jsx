      ...
      {nextMeeting && (
        <Card sx={{ mb: 3, bgcolor: '#F0FDF4', border: '1px solid #BBF7D0' }}>
          <CardContent>
            ...
            <Button variant="contained" size="small" color="success" sx={{ mt: 1.5 }} onClick={() => navigate(`/sitting/${nextMeeting.id}`)}>
              Join Sitting & Vote
            </Button>
          </CardContent>
        </Card>
          ...
          <Card>
            <CardContent sx={{ py: 2.5 }}>
              ...
              {isLoading ? <Skeleton width={40} height={40} /> : (
                <Typography variant="h1" sx={{ color: '#059669', fontWeight: 700 }}>{meetings.length}</Typography>
              )}
            </CardContent>
          </Card>
        </Grid>
